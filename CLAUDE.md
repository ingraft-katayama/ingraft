...
**変更したい場合**: ファイルを編集してコミット・プッシュすれば、全員に反映されます。
...
**変更したい場合**: 自分のパソコンで直接編集します。他のメンバーには影響しません。

### どっちに書けばいいか迷ったら

- **全員に共通のルール** → プロジェクト（`.claude/settings.json` 等）
- **自分だけのAPIキーやトークン** → 個人（`~/.zshrc` の環境変数）
- **自分だけの追加設定** → 個人（`~/.claude/settings.json`）
