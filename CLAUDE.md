...
以下のファイルを追加・変更・削除した場合、作業完了後に **必ず** `#claude`チャンネル（ID: `C0AQ279HQKH`）へ `@channel` メンション付きで通知すること。
...
**変更内容:** （何が変わったかを簡潔に）
**変更ファイル:** （対象ファイル名）
**影響:** （チームメンバーが知っておくべきこと・必要なアクション）
**変更者:** （分かれば記載）
```

### ルール
- 通知は変更の **コミット後** に1回だけ送る（編集中の途中では送らない）
- 複数ファイルをまとめて変更した場合は1つの通知にまとめる
- 軽微な修正（typo修正など）でも対象ファイルであれば通知する
